...
- 执行顺序按最新要求调整为：本地不再跑训练；先完成 X4 云端提升训练和全量 REDS 评估打包，再在同一云端服务器启动 X2 提升训练；训练阶段不启动 X2 Vivado/JTAG。
...
- X2 云端接力 watcher 已接入：`scripts/cloud/watch_x4_package_then_start_x2_training.py`。它等待 X4 候选 `manifest.json` 出现后，在云端启动 X2 `scale=2, c32b4` 提升训练，不在本地跑训练，不启动 Vivado/JTAG/XSCT/板卡访问。
