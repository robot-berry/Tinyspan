...
- 执行顺序按 2026-06-26 最新节点调整为：先完成 X4 云端提质训练和 full-val 软件门禁；X4 未达 `28dB` 时保留当前 X4 Gate H 硬件提交基线，不进入量化/RTL/上板替换流程；随后启动 X2 云端提质训练。
- X4 提升训练支线目标是超过当前 X4 软件基线和 bicubic，再争取 REDS full-val `>=28dB`；`QUALITY_X4_HRHEAVY_P256_20260626` 已完成但 full-val PSNR 为 `26.384690521945394dB`，未达到替换门槛，不替换当前 `X4_SUBMIT_20260625_CURRENT_BASELINE`。
- X4 训练后自动评估 watcher 已接入；本轮主 watcher 在训练后续跑阶段出现 shell 引号解析错误，恢复 watcher 已补跑 X4 full-val 和候选包打包。后续重新启动 X4 提升训练时，必须使用新的 run ID 和 candidate ID，并在训练完成后运行全量 REDS `val_sharp` 软件质量评估并打包候选证据，不启动 Vivado/JTAG/XSCT/板卡访问。
- X2 本地原训练保留在 epoch `38`、step `147107 / 198000`，其 `student_latest.pt` 只作为云端 X2 resume 起点；本地 X2 训练进程已按最新要求停止。
- 当前 X2 云端提升训练已经启动并由恢复 watcher/X2 打包兜底 watcher 接管；后续等待 X2 `student_last.pt`、full-val 质量包和候选 manifest。
...
- X4 后续画质提升继续作为支线推进；已跑过的 `x4_quality_after_x2_20260625_cloud_eval` 和 `QUALITY_X4_HRHEAVY_P256_20260626` 都未达到替换门槛，不替换当前 X4 submission baseline。新的 X4 提升训练只有重新完成冻结、量化、RTL/export、bitstream、真实板上 `0 mismatch` 和 `>=30fps` 后，才允许成为替代提交节点。
...
- 恢复 watcher：`/root/autodl-tmp/Tinyspan/logs/cloud/recover_x4_x2_after_watcher_error_20260626.sh`。原因：主 watcher
  在 X4 训练完成后的 shell 续跑阶段出现引号解析错误；恢复 watcher 已完成 X4 full-val/打包，并已启动 X2 提质训练。
