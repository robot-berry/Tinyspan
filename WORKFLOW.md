...
- 执行顺序按最新要求调整为 X4 云端提升训练与 X2 本地提升训练并行推进；训练阶段不启动 X2 Vivado/JTAG。
- X4 云端全量训练已使用 `train_sharp` 全量配置启动，目标是先超过当前基线和 bicubic，再争取 `>=28dB`。
- X4 训练后自动评估 watcher 已接入：`scripts/cloud/watch_x4_training_then_eval.py`。它在训练完成后运行全量 REDS `val_sharp` 软件质量评估并打包候选证据，不启动 Vivado/JTAG/XSCT/板卡访问。
- X2 本地原训练保留在 epoch `38`、step `147107 / 198000`；已从该 run 的 `student_latest.pt` 启动新的 X2 提升续训 run：`runs/tinyspan_distill/video_x2_c32_b4_reds_temporal_quality_resume_20260625`。
- X2 提升续训目标为剩余约 `13` 个 epoch、`51480` step；完成后再用同一 checkpoint 闭合 X2 freeze/quant/RTL/board，并评估 REDS val 平均 PSNR 是否达到 `>=30dB`。
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\run_tinyspan_c32b4_post_training_prep.ps1 `
  ...
  -Scale 2 `
  -Tag x2_quality_resume_YYYYMMDD
```

X4 路线使用同一个入口，只把 `-Scale` 改为 `4`，并使用对应的 X4 训练输出目录。若从
`G:\UESTC\feitengspan1\Tinyspan` 镜像仓库内执行脚本，`-RunDir` 应使用 `..\runs\...` 或主工程中的
绝对训练目录，例如 `G:\UESTC\feitengspan1\runs\tinyspan_distill\video_x2_c32_b4_reds_temporal_quality_resume_20260625`；
