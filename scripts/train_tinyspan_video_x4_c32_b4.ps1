...
try {
  if ($Smoke) {
    $Output = "runs\tinyspan_distill\video_reds_smoke_x4_c32_b4"
    $PatchSize = 192
    $BatchSize = 1
    $Epochs = 1
    $MaxPairs = 24
  ...
  Write-Host "TINYSPAN_C32B4_TRAIN_OUTPUT=$Output"
  Write-Host "TINYSPAN_C32B4_TRAIN_COMMAND=python $($argsList -join ' ')"
  python @argsList
  if ($LASTEXITCODE -ne 0) {
    throw "C32B4 TinySPAN video distillation failed with exit code $LASTEXITCODE"
  }
  ...
  Write-Host "TINYSPAN_C32B4_CHECKPOINT=$checkpoint"
  Write-Host "TINYSPAN_C32B4_PREVIEW=$preview"
  Write-Host "PASS train_tinyspan_video_x4_c32_b4"
}
finally {
  Pop-Location
}
