...
try {
  ...
  $projDir = Join-Path $root "build\vivado_sr_stream_dynamic_cropper_sim"
  $simLog = Join-Path $projDir "sr_stream_dynamic_cropper_sim.sim\sim_1\behav\xsim\simulate.log"
  $tcl = Join-Path $scriptDir "run_vivado_sim_sr_stream_dynamic_cropper.tcl"
  $runStarted = Get-Date
  ...
  if ($simText -notmatch "PASS sr_stream_dynamic_cropper" -or
      $simText -match "(?m)(Fatal:|ERROR:|MISMATCH)") {
    throw "sr_stream_dynamic_cropper simulation did not pass; see $simLog"
  }
  ...
  Write-Host "PASS run_vivado_sim_sr_stream_dynamic_cropper"
} finally {
  Pop-Location
}
