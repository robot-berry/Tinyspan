#!/usr/bin/env python3
"""Watch cloud X2 training, then run the REDS HR software quality gate.

This helper only uses SSH/SFTP and Python evaluation scripts. It can be started
before cloud X2 training exists; it will wait for the run to appear, then wait
for training completion. It does not start Vivado, JTAG, XSCT, board access,
...
DEFAULT_QUALITY_DIR = "runs/tinyspan_quality/x2_quality_after_x4_reds_val"
DEFAULT_CANDIDATE_ID = "x2_quality_after_x4_20260625"
DEFAULT_ARTIFACT_ROOT = "artifacts/20260618_x4_tinyspan_c32b4_baseline_30fps_safe/x2_quality_candidates"
...
python scripts/acceptance/package_x4_quality_candidate.py \
  ...
  --allow-incomplete
"""
    print("RUN_REMOTE_X2_EVAL_AND_PACKAGE", flush=True)
...
def download_artifacts(client: paramiko.SSHClient, remote_repo: str, remote_artifact_dir: str, local_dir: Path) -> None:
    ...
    try:
        ...
        for remote_file in sftp_walk(sftp, base):
            rel_path = posixpath.relpath(remote_file, base)
    ...
    try:
        while True:
            ...
            if state["package_manifest_exists"] and not args.force_eval:
                print("X2_PACKAGE_ALREADY_EXISTS", flush=True)
            ...
            if state["process_count"] > 0:
                if time.time() >= deadline:
                    raise TimeoutError("Timed out waiting for X2 cloud training to finish.")
                time.sleep(max(args.poll_seconds, 1))
                continue

            if not state["metrics_exists"] and not state["student_last_exists"]:
                if time.time() >= deadline:
                    raise TimeoutError("Timed out waiting for X2 cloud training to start.")
                print("WAIT_X2_CLOUD_TRAINING_START", flush=True)
                time.sleep(max(args.poll_seconds, 1))
                continue

            if not state["student_last_exists"]:
                raise RuntimeError("X2 training process stopped before student_last.pt was written.")

            if args.min_steps > 0 and int(state["step"]) < args.min_steps:
                raise RuntimeError(
                    f"X2 training stopped early at step {state['step']} < required {args.min_steps}."
                )
